...
export class RegrasComponent implements OnInit {
  ...
  private todasAsRegras: RegrasContent = {
    quotas: [
  ...
  // Modificação aqui: receba o evento completo
  selecionarTipoRegra(event: Event): void {
    const target = event.target as HTMLSelectElement; // Faça o type assertion aqui
    if (target) { // Opcional, mas boa prática: verificar se target não é null
      const tipo = target.value;
      this.tipoRegraSelecionado = tipo;
      this.regrasExibidas = this.todasAsRegras[tipo] || [];
    }
  }
}